...
/*
    ...
    SEARCH UI APP
    =======================================
    delimiters:["[[","]]"] Indicates the delimeters 
               are [[...]] instead of the default {{...}}
...
const searchUi = new Vue({
    ...
    methods: {
        digestUrlQueryStr(){
            ...
            // Redpopulate this.query Obj based on url address str
            // Group Headers First
            // Split off the comma sep'd stuff
            let segs = self.url_qry_str.split(',');
            // Assign the 1st portion of url to iVault Group Header string
            let groups_str = segs[0];

            console.log('SEGS:',segs);
            let groupHeaders = groups_str.split('^');
            groupHeaders.shift(); // remove the 1st array elem as it will be empty
            console.log('groupHeaders',groupHeaders);
            // Process the Group Headers First
            ...
            // Build the iVault Options Definiton groupings
            //     Shift the Group Headers string off the array
            //     so the remaining numerical IDs can be iterated
            segs.shift();
            segs.forEach(function(def,i){
                if(def !== ''){
                    // ~ indicates an AND'd OR
                    if(def.charAt(0) == '~'){
        ...
        urlQueryStringBuilder(){
            str = '';
            self = this;// Avoid the "scope gatcha"
        ...
        urlQueryVerbalizer(){
            self = this;// Avoid the "scope gatcha"
            this.query.forEach(function(union, i){
                console.log('Verbalizer: union('+i+')  ', union);
        ...
        async get_opt_grp_definintions(e){
            ...
            .then(function(response){
                self.current_option_header = response.data[0]['groupName'];
                self.grp_defs = response.data;
                console.log('response.data:', response.data);
                console.log('self.current_option_header', self.current_option_header);
                // 
            })
